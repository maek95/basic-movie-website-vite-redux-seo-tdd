...
export const visitedMoviesSlice = createSlice({
  ...
  reducers: {
    ...
    addToVisitedMovies: (state, action) => {
      ...
      isAlreadyVisited = visitedMovies.some((movie) => movie.id == movieObject.id )
      
      if (isAlreadyVisited === false) {
        console.log("new movie visited, adding to visitedMoviesArr");
        
        const updatedVisitedMovies = [...state.visitedMoviesArr, action.payload] // even though we have Immer this seems required to post correct updated favourites to localStorage.
        state.visitedMoviesArr = updatedVisitedMovies;
        // localStorage not accessible during build
        if (typeof window !== "undefined") { 
          localStorage.setItem("visitedMovies", JSON.stringify(updatedVisitedMovies)); // can't store arrays directly, need JSON string.
        }
      } else {
        console.log("Already visited movie " + action.payload.title + " won't add to visitedMoviesArr");
      }      
...
export const { setVisitedMoviesFromLocalStorage, addToVisitedMovies, removeFromVisitedMovies } = visitedMoviesSlice.actions;
