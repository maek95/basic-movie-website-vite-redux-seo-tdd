...
} from "../redux/FavouritedMoviesSlice";
...
export default function MovieCard({ movieObject }) {
  ...
  useEffect(() => {
    ...
    const isFavourite = favouritedMoviesArr.some(
      (favourite) => favourite.id == movieObject.id
    );
  ...
  return (
    <div className="border-[0.5px] border-solid border-white  p-4 w-32 h-96 break-words flex flex-col justify-between">
      ...
      <Link
        className="flex flex-col MovieLink box-border  w-32 text-inherit no-underline hover:text-inherit hover:opacity-55"
        to={`/movie/${movieObject.id}`}
      >
        <h3 className="whitespace-normal break-words font-semibold sliced-text h-14 w-full">
          {movieObject.title}
        </h3>
        <img
          ...
          src={`${
            movieObject.poster_path
              ? `${baseImageUrl}${movieObject.poster_path}`
              : `${
                  movieObject.backdrop_path
                    ? `${baseImageUrl}${movieObject.backdrop_path}`
                    : "/clapperboard-movie-placeholder.png"
                }`
          }`}
          alt=""
        />
